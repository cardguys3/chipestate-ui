...
export default function VoteDetailPage() {
  ...
  async function fetchUserVote() {
    ...
    if (!userId) return;
    const { data } = await supabase.from("user_votes").select("option_id").eq("vote_id", id).eq("user_id", userId).single();
  ...
  async function castVote(optionId) {
    ...
    // Count chips owned right now
    const { data: chips } = await supabase.from("chips").select("id").eq("owner_id", userId);
    const chipCount = chips?.length || 0;

    if (chipCount === 0) return alert("You must own chips to vote.");
    ...
    <div className="p-6 max-w-2xl mx-auto">
      ...
      <ul className="space-y-2">
        {options.map(opt => {
          ...
          const percent = Object.values(tallies).reduce((a, b) => a + b, 0) > 0
            ? (totalVotes / Object.values(tallies).reduce((a, b) => a + b, 0)) * 100
            : 0;

          return (
            <li key={opt.id} className="p-3 border rounded-lg flex justify-between items-center">
              <div>
                <p className="font-medium">{opt.label}</p>
                {!vote.is_open && (
                  <p className="text-sm text-gray-500">{totalVotes} chips ({percent.toFixed(1)}%)</p>
                )}
              </div>
              {vote.is_open && (
                <Button
